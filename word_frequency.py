...
def print_word_freq(file):
    ...
    with open(file) as text:
        ...
        # Removes any words contained in STOP_WORDS from the list of words
        print(word_list)
        # This list variable is initialized so we can append strings to it in the following for loop
        word_count_list = []
        # In this loop, the count method is called on word_list to count instances of a word, and saved to the
        # word_count variable as a string. The list created above has an entry appended that includes the
        # word, number of times it appears, and asterisks equal to that number. This list contains duplicate
        # entries for each time the word appears, and is not sorted.
        for word in word_list:
            word_count = str(word_list.count(word))
            word_count_list.append(word + " | " + word_count + " " + ('*' * int(word_count)))
